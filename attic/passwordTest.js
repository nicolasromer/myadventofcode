...
const raw = `13-15 c: cqbhncccjsncqcc
2-3 v: zvdvfd
...
const convertInput = rawInput => rawInput.split('\n').map(rawline => {
	const splitCase = rawline.split(' ');
	const range = splitCase[0].split('-');
	splitCase[0] = range[1];
	splitCase.unshift(range[0]);
	splitCase[2] = splitCase[2].replace(':', '');
	return splitCase;
});
...
const testPassword = (pw) => {
	...
	const matches1 = password[pos1 - 1] === letter ;
	const matches2 = password[pos2 - 1] === letter 
	const valid = (matches1 && !matches2) || (matches2 && !matches1)

	return valid;
}

const tallyCorrectPasswords = passwords => {
	let tally = 0;
...
const runTest = () => {
	convertInput(raw).forEach(pass => {
		const passed = testPassword(pass);

		if (passed) {
			console.log('input ' + pass + ' is ' + (passed ? 'VALID' : 'NOT') );
		}
	})
}
...
window.onload = () => {
	var t0 = Math.floor(performance.now())

	const passwords = convertInput(raw);
	console.log('Total passwords: ' + passwords.length);
	answer = tallyCorrectPasswords(passwords);

	
	var t1 = Math.floor(performance.now())
	document.body.textContent = 'Answer: ' + answer + '.  Took :' + (t1 - t0) + " miliseconds."
}
