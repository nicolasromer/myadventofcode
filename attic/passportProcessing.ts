...
const fieldsRegex: RegExp = /(ecl|pid|eyr|hcl|byr|iyr|cid|hgt):(\S*)/gi
...
const processPassportData: (rawPassportData:string)=> Array<Passport> = function(rawPassportData) {
	const rawPassports: Array<string> = rawPassportData.split('\n\n').map(p => p.trim());
	console.log('1: ', rawPassports[0]);

	const capturedPassports = rawPassports.map((rp) => [...rp.matchAll(fieldsRegex)]);
	console.log('2: ', capturedPassports[0]);

	const arrayedFields = capturedPassports.map(captured => {
		return captured.map(capturedField => capturedField.slice(1,3));
	})
	console.log('3: ', arrayedFields[0]);

	const passports = arrayedFields.map(fields => {
...
const isYearInRange = (input, min, max): boolean => {
	if (!isNumber(input)) return false;
	const year = parseInt(input, 10);
	return year >= min && year <= max; 
};

const cm = "(1[5-8][0-9]|19[0-3])cm"; // 150 - 193
const inches = "(59|6[0-9]|7[0-6])in"; // 59-76
const heightRegxp = new RegExp(`^(${cm}|${inches})$`);
const isValidHeight = input => heightRegxp.test(input);

const isValidHairColor = input => /^#[0-9a-f]{6}$/.test(input);
const isValidEyeColor = input => /^(amb|blu|brn|gry|grn|hzl|oth)$/.test(input);
const isValidPassportId = input => input.length === 9 && isNumber(input);

const evaluatePassport: (passport: Passport) => boolean = function(passport) {
	const { ecl, pid, eyr, hcl, byr, iyr, cid, hgt } = passport;
	
	// 'cid' can be ignored

	if (!(ecl && pid && eyr && hcl && byr && iyr && hgt)) return false;
...
(()=> {
	const passports = processPassportData(invalidPassports);
	const result = passports.map(passport => evaluatePassport(passport));
	const pass = result[0] === false
		&& result[1] === false
		&& result[2] === false
		&& result[3] === false;
	...
	console.log('INVALID: ', 
		pass ? 
		'PASS' : 'FAIL. Actual: ' + result);
})();

(()=> {
	const passports = processPassportData(validPassports);
	const result = passports.map(passport => evaluatePassport(passport));
	const pass = result[0] === true
		&& result[1] === true
		&& result[2] === true
		&& result[3] === true;
...
document.body.innerHTML = `<h1 style='font-family:"helvetica"'>
${(()=>{
	console.log('');
	console.log('FINAL RUN');
	const passports = processPassportData(realData);
	const result = passports.map(passport => evaluatePassport(passport));
