"use strict";
const fieldsRegex = /(ecl|pid|eyr|hcl|byr|iyr|cid|hgt):(\S*)/gi;
const processPassportData = function (rawPassportData) {
    const rawPassports = rawPassportData.split('\n\n').map(p => p.trim());
...
const isYearInRange = (input, min, max) => {
    if (!isNumber(input))
        return false;
...
const evaluatePassport = function (passport) {
